...
function instrumentFfmpeg(ffmpeg) {
  ...
  // exec() — second arg is timeout in seconds in 0.12.7. Default it
  // to 30s so an unbounded call cannot hang forever.
  const _exec = ffmpeg.exec.bind(ffmpeg);
  ffmpeg.exec = function(args, timeoutMs, signal) {
    let ms = (typeof timeoutMs === 'number' && timeoutMs > 0) ? timeoutMs : TT.execExec;
    // ffmpeg.wasm's exec takes (args, timeout in seconds, signal).
    // Convert ms → s and never pass 0 / negative.
    const sec = Math.max(0.001, ms / 1000);
    return _exec(args, sec, signal);
  };
...
async function initFFmpeg() {
  ...
  // v5 hotfix 6: try ST first (MT has a function signature mismatch in
  // the pthread worker that crashes multi-frame encodes). ST is the
  // safe, well-tested path. If the user wants MT, they can re-enable
  // it by uncommenting the canMT check below.
...
async function executeFFmpeg(args) {
  ...
  try {
    ...
    return { data: u8, mime, filename: outputFilename };
  } catch (err) {
