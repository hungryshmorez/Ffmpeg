...
(function () {
  ...
  let srcVideoEl = null;
  const cues = [null, null, null, null];
  ...
  function build() {
    ...
    tab.innerHTML = `
      <div class="vj-stage">
        ...
        <div class="vj-hud">
          <span id="perf-hud" class="perf-hud"></span>
      ...
      <div class="vj-deck">
        <div class="vj-transport">
          ...
          <label class="vj-master">
            <span>MASTER</span>
            <input type="range" id="vj-master" min="0" max="1" step="0.01" value="1">
          </label>
          <select id="vj-quality" class="ctrl" title="Auto sheds load when the frame rate drops">
  ...
  function bind() {
    ...
    // MASTER — one knob over everything. When it's all too much you want ONE
    // control, not twenty-three.
    const mst = document.getElementById('vj-master');
    mst.addEventListener('pointerdown', () => window.FFPerf.Master.capture(engine));
    mst.addEventListener('input', (e) => window.FFPerf.Master.set(+e.target.value, engine));
    mst.addEventListener('pointerup', () => window.FFPerf.Master.release());

    document.getElementById('vj-quality').addEventListener('change', (e) => {
      const v = e.target.value;
      if (v === 'auto') { window.FFPerf.Perf.auto = true; log('Quality: auto — it will shed load if the frame rate drops.'); }
      else { window.FFPerf.Perf.setTier(v); log(`Quality locked to ${v}.`); }
    });

    // HOT CUES — jump points in the source. Click to jump, shift-click to set.
