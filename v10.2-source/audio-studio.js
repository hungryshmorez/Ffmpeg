...
(function () {
    ...
    const loop = () => {
      ...
      if (spec) {
        ...
        for (let i = 0; i < bars; i++) {
          // log-ish frequency mapping — looks right to a musician
          const idx = Math.floor(Math.pow(i / bars, 2) * spec.length);
          const v = spec[idx] / 255;
          const bh = v * h * 0.92;
          const hue = 190 - v * 90;                 // cyan → magenta as it gets loud
  ...
  async function bounce() {
    ...
    try {
      ...
      const rendered = await eng.bounce((p) => { status.textContent = `Rendering… ${Math.round(p * 100)}%`; });
