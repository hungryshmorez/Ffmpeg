...
async function analyzeMedia(virtualFilename) {
  ...
  try {
    ...
    // Actually remove OUR listener. The previous code added a fresh no-op
    // listener instead of removing onLog, so every probe leaked a listener
    // and `captured` kept growing across probes.
    try { state.ffmpeg.off('log', onLog); } catch (_) {}
  }

  const stderr = captured.join('\n');
  const meta = parseFFmpegStderr(stderr);
  // Stream presence, straight from the probe's stderr. This is the ground
  // truth the split-render pipeline and the audio guard rely on — the
  // upload-time `hasAudio` default is just a guess (true for any video),
  // and NOT updating it here was why a video-only clip still tried (and
  // failed) to run an audio pass.
  ...
  if (state.inputFile) {
    ...
    // Only trust a POSITIVE audio finding to flip hasAudio off; if the probe
    // saw no streams at all (empty stderr) leave the prior value alone.
    if (hasVideoStream || hasAudioStream) {
      state.inputFile.hasAudio = hasAudioStream;
      state.inputFile.hasVideo = hasVideoStream;
      state.inputFile._hasAudioStream = hasAudioStream;   // cached for the render-time guard
    }
  }
    ...
    for (let i = 0; i < totalSteps; i++) {
      ...
      try {
        ...
        if (_rc !== 0 && _rc !== undefined && _rc !== null) {
          throw new Error(`ffmpeg exit code ${_rc}`);
        }
        stepOk = true;
      } catch (e1) {
        ...
        for (let r = 0; r < retryStrategies.length; r++) {
          ...
          try {
            const _rc2 = await ff.exec(rArgs);
            if (_rc2 !== 0 && _rc2 !== undefined && _rc2 !== null) {
              throw new Error(`ffmpeg exit code ${_rc2}`);
            }
            stepOk = true; fellBack = true; break;
          } catch (e2) {
...
async function executeWithRetry(args, retryStrategies) {
  ...
  for (let i = 0; i < attempts.length; i++) {
    ...
    try {
      ...
      const _rc3 = await ff.exec(a.args);
      if (_rc3 !== 0 && _rc3 !== undefined && _rc3 !== null) {
        throw new Error(`ffmpeg exit code ${_rc3}`);
      }
      // Read the output
