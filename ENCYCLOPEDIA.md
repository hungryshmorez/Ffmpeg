...
| Status | Count | Items |
|---|---|---|
| ✅ **Done** (verified by running) | **87 / 100** | everything not listed below |
...
| 🔒 **Blocked** (needs real hardware/toolchain) | 6 | #14 OffscreenCanvas/Worker shaders · #16 WASM SIMD SAD · #65 true DCT · #79 NDI/virtual-cam · #84 shot-type classify · #87 content-aware fill |
...
is complete. Motion estimation runs off-thread (#15), the shaders link and render on strict GL
(the varying-mismatch fix), which unblocked the program cache (#19), and the WebCodecs path is
codec-adaptive and verified (#13). The GPU-thread render move (#14) still can't be pixel-verified
without a real GPU adapter (`navigator.gpu.requestAdapter()` returns null here), same bar as the
WebGPU work. The architecture items (#97–99) are pure refactors with no user-facing capability,
so they're deliberately deferred over destabilising the working app.
...
| **Live FX preview + Record → Bin** | ✅ | `preview-fx.js` (`FFPreviewFX`) — a "✨ Live FX" mode in the Preview tab pipes the source frame-by-frame through the shared effect engine (FFHoverPreview look ops **+ real FFShaderPlus shader passes**: pixel sort, halation, film grain — the same library Trip Cam uses) onto `#pv-fx`, with an effect picker + amount, and a **Record → Bin** button that captures the canvas via `TripCam.startRec` straight into the Media Bin. Answers "can the preview screen do the same things — apply effects live and record them so ffmpeg can stack more on top." `.test/preview-fx.mjs` (6/6) verifies the effect list (incl. shader passes), the pixel ops (grayscale→R=G=B, invert→255−x, vibrant widens spread), that the shader passes actually alter pixels, that `renderFrameFrom` paints the effect onto `#pv-fx`, that entering/leaving the mode starts/stops the loop, and end-to-end that Record → Bin lands a **decodable** 480×270 clip in the bin. |
...
| # | Feature | What it is / the vision | Status | What's left |
|---|---|---|---|---|
| 13 | Ship the WebCodecs path | Direct GPU/ASIC encode — 10–50× over wasm. | ✅ | Now **codec-adaptive** and verified. The old path hardcoded H.264 and simply failed on the many machines/browsers that ship no H.264 *encoder*; `pickEncodeCodec()` chooses the best AVAILABLE encoder (H.264 → VP9 → AV1 → HEVC, with the matching mp4-muxer tag) and `hwTranscode` uses it. `probeCodecs` now records both HARDWARE-backed support (`prefer-hardware` → drives routing) and usable-at-all support (`no-preference` → `encodeAny`/`decodeAny`). `FFHardware.hwSelfTest()` is the WebCodecs analogue of the wasm 🩺 self-test: encode N synthetic frames through the real `VideoEncoder` and decode them back through `VideoDecoder`. `.test/hw-webcodecs.mjs` (4/4): the picker skips H.264 when absent and lands on VP9 here, and a 24-frame round trip preserves the frame count AND the pixels — a moving pink marker on a dark bg survives 24/24. (Real H.264 *hardware* still needs a GPU-enabled runner; the codec path + adaptivity are proven.) — *M* |
...
| 19 | Cache compiled shader programs | We recompile 11 shaders per canvas. | ✅ | A per-GL-context program cache keyed by (patched) shader source: `_compileAll` reuses linked programs instead of recompiling, and the cache is evicted on context loss (the programs die with the context). Now verifiable because the shaders LINK (the varying-mismatch bug is fixed). `.test/shader-cache.mjs` (5/5): engine A links all 11 programs; a 2nd engine sharing the canvas/context makes **zero** new `compileShader` calls, holds the **same** program objects, and RENDERS non-black through them; an engine on a new canvas recompiles (the cache is correctly per-context, not a global handing back dead programs). Cross-canvas engines still get separate GL contexts (a WebGL constraint), so the everyday win is a shared-context rebuild. |
...
| 32 | Automatic gain staging | Warn when the rack clips into the reverb. | ✅ | `FFAudioDSP.gainStaging(params, inputPeakDb)` walks the rack's boosts in series (bass → EQ → distortion → output) and flags the FIRST stage whose running level crosses 0 dBFS, so the warning names the culprit (the bass boost, not the master). Surfaced live in the Audio Studio (`#as-stage-warn`) as params change. `.test/gain-staging.mjs` verifies a +12 dB bass boost on a −3 dB input clips at the bass stage, a flat rack on −6 dB stays clean, and Volume>1 clips the output stage. |
| 33 | A/B vs a reference track, loudness-matched | Honest comparison. | ✅ | `FFAudioDSP.loudnessMatch(mix, reference)` returns the gain that brings the mix onto the reference's RMS (louder always sounds "better", so an honest A/B must match level). An "⚖ A/B a reference" button loads a track and toggles playback of it at the matched gain. `.test/gain-staging.mjs` verifies matching a −0.2 mix to a −0.5 reference gives a 2.5× gain that equalises the RMS, and matching a signal to itself is unity. |
...
| 53 | Curves editor with a draggable spline | Not preset names. | ✅ | `FFShaderPlus.buildCurveLUT(points)` turns control points into a 256-entry tone LUT (piecewise through them, endpoints filled); `applyCurve` maps a frame's channels through it — master (RGB) or per-channel R/G/B for split-tone. `renderCurve` applies a curve per frame → Media Bin; workflows "📈 Film Contrast" / "🎞️ Faded Matte" ship real curves (the same spline widget the speed panel uses drives it in the UI). `.test/curves.mjs` verifies the identity no-op, a control point hitting exactly (128→190), invert = 255−i, and a per-channel curve leaving the other channels alone. |
...
| 56 | Frame-blend vs optical-flow toggle | Choose retime method. | ✅ | `MotionMosher.retime(a,b,vec,…,t,method)` produces the in-between frame either way — `'flow'` warps along the motion (one sharp object at the in-between position, via `interpolate`) or `'blend'` cross-dissolves (the cheaper frame-mix, moving objects ghost). Surfaced as two labelled slow-mo workflows ("🐢 optical flow" / "🐌 frame blend"). `.test/retime-toggle.mjs` verifies at t=0.5 that flow puts one block at the midpoint (255, ends dark) while blend leaves two ghosts (128 at each end, dark midpoint). |
...
| # | Feature | What it is / the vision | Status | What's left |
|---|---|---|---|---|
| 69 | MIDI clock sync (slave) | Play with anyone. | ✅ | `FFMidiIn` — locks the VJ sequencer to an INCOMING MIDI clock. Decodes the realtime stream (24-PPQN clock 0xF8, start 0xFA, continue 0xFB, stop 0xFC), counts 6 pulses per 16th step, derives BPM from the pulse interval (EMA-smoothed), and drives the sequencer one step at a time. VJ's `tick()` was split into a schedulable `stepBody`; new `extStart`/`extStop`/`stepTick`/`setExternalClock` let an outside clock replace the internal `setTimeout` scheduler, and `slaveVJ(true)` wires START→`extStart`, STOP→`extStop`, each 16th→`stepTick`, tempo→`S.bpm`. `init()` attaches to Web MIDI inputs when present; idle otherwise. `.test/midi-in.mjs` (5/5) verifies 6-pulses-per-step (24 clocks → 4 steps), transport (STOP halts, clock does nothing while stopped, CONTINUE resumes), BPM derivation (20.83 ms → 120), a raw `{data:[…]}` event shape, and end-to-end that slaving the real VJ steps its playhead 0→4 over 24 clocks and STOP halts it. Existing VJ tests (pattern-banks, launch-quantize, beat-chop) still green. *(Receiving real hardware bytes is the only device-gated part.)* |
...
| 71 | MIDI output | Send clock/notes so lights follow. | ✅ | `FFMidiOut` — sends MIDI from the VJ deck so external gear follows: a 24-PPQN MIDI clock locked to BPM (`startClock`/`continueClock`/`stopClock` → 0xFA/0xFB/0xF8/0xFC) and note-on/off whenever a trigger fires. Pure encoders (`noteOn`/`noteOff`/`cc`, channel-masked + data-clamped) and `clockIntervalMs = 60000/(bpm·24)`. Trigger→note mapping is deterministic (`mapTriggers` seeds from the sequencer table). `init()` uses Web MIDI when present and picks the first output; everything degrades gracefully to a log-only mode when there's no device/permission (`send()` never throws). Wired into VJ `fire`/`release` (note-on/off) and `panic` (all-notes-off + stop clock). `.test/midi-out.mjs` (6/6) verifies the encoders + realtime constants, 24-PPQN timing, note-on/off to an injected output with a deterministic map, a START→CLOCK…→STOP stream, that disabled sends nothing and a missing output never throws, and end-to-end that firing/releasing a real VJ trigger emits note-on/off. *(Physical Web-MIDI delivery to hardware is the only device-gated part.)* |
...
| 73 | Per-layer effect chains | Each layer its own shader stack. | ✅ | `FFFxChain` — an ordered STACK of effects (each stage its own effect + amount + on/off) applied top-to-bottom, so you can layer e.g. VHS → Vignette → Film Grain. Each stage delegates to `FFPreviewFX.applyEffect`, so the chain inherits every Live-FX effect (look ops + real FFShaderPlus shader passes). `add`/`remove`/`move`/`toggle`/`setAmount`/`clear`, `serialize`↔`restore`. The default `preview` chain drives the Live FX surface — reorderable chips with per-stage enable/remove in the FX panel, `＋ Add to chain` from the picker — and takes precedence over the single picked effect; it also registers as a #89 undo provider so Ctrl+Z steps through chain edits. (More `Chain` instances can drive individual compositor layers.) `.test/fx-chain.mjs` (7/7) verifies order matters ([grayscale,warm]=coloured vs [warm,grayscale]=grey), disabled stages skip / all-disabled is a no-op, the mutations, serialize↔restore, and end-to-end that a stacked chain drives the preview loop over the single picker and the ＋ Add / remove UI works. |
...
| 75 | Pattern banks (8, switch on the bar) | Recall sequencer patterns. | ✅ | Eight bank slots below the sequencer — `saveBank(i)` stores a deep clone of the current 16-step pattern, `recallBank(i)` restores it (immediate when stopped, QUEUED to the next bar via the `S.pendingBank` check in `tick()` when playing, so the switch lands on the downbeat). Click a bank to recall, Shift+click to save. `.test/pattern-banks.mjs` drives the real deck: save→change→recall restores the pattern, the recall is a deep copy (editing it doesn't mutate the bank), an empty bank is a safe no-op, and a recall while playing queues rather than applying instantly. |
| 76 | Automation recording | Record knob moves, play back. | ✅ | `FFAutomation.Automation` records parameter moves stamped relative to the take start; `valueAt`/`stateAt` read it back sample-and-hold (the last move ≤ t), and it serialises. The VJ deck wires ⏺ REC / ▶ AUTO buttons — REC captures MASTER-fader moves, AUTO re-schedules them. `.test/automation.mjs` verifies record() is a no-op until a take starts then stamps relative time, sample-and-hold playback (undefined before the first move), full-patch `stateAt`, and a serialize/deserialize round-trip. |
...
| 85 | Auto colour-match across clips | You have the LUT generator; run it clip-to-clip. | ✅ | `FFShaderPlus.matchColorStats` applies a direct Reinhard mean/std transfer (per channel, with a strength blend); `renderColorMatch` grabs a reference frame from clip A and grades clip B to its look, frame by frame → Media Bin. Wired as a "🎨 Match Colour A→B" bin-op (select 2, mirroring Datamosh A→B). `.test/color-match.mjs` verifies the target's per-channel mean lands on the reference's (e.g. [165,113,75]), the std matches too, strength=0 is a no-op / 0.5 halfway, and self-match is identity. |
...
| # | Feature | What it is / the vision | Status | What's left |
|---|---|---|---|---|
| 89 | Undo/redo across the WHOLE app | Not just the editor. | ✅ | The existing form-control undo stack now carries **non-DOM module state** too. `window.registerUndoProvider(name, {capture, restore})` lets any module ride the shared stack: each snapshot embeds `_captureCustom()` under a reserved `__custom` key (deep-compared for dedupe), and `_applySnapshot` calls `_restoreCustom` on undo/redo; `capture()→null` opts a module out of a snapshot. `window.scheduleUndoSnapshot`/`pushUndoSnapshot` are exposed so a module flags its own changes. Wired for real on the **VJ sequencer pattern** (a non-DOM `{trigger:[bool×16]}` object): editing a step flags a snapshot, and its provider deep-clones/restores the pattern + re-syncs the grid. `.test/undo-custom.mjs` (4/4) verifies a provider's state stays in lockstep with a control across a run of undo/redo (never a mismatched pairing) and round-trips to the start, that a null capture opts out without breaking undo, and end-to-end that app-level Undo reverts a real VJ step edit while keeping the grid DOM and the pattern state consistent (and Redo re-applies it). |
| 90 | Real onboarding tour | Four dismissible, `localStorage`-gated steps. | ✅ | `FFOnboarding` — a four-step spotlight tour (add media → pick a recipe → search & hover-preview → fine-tune & export). Each step anchors to a real element by selector; a giant box-shadow on the spotlight ring dims the rest, and the tooltip auto-places below/above the target (centres if the target is missing). Auto-starts once on first run *after* the mode picker is gone, gates itself in `localStorage` (`ffstudio.tour.v1`) on finish OR skip so it never nags, switches to a step's target tab as it lands there, is keyboard-driven (→/Enter/←/Esc), and is replayable from a "Take the tour" button added to the keyboard cheat sheet. `.test/onboarding.mjs` (6/6) verifies auto-appearance at step 1 with the spotlight on the Add-media button, that Next advances the text/index and activates the editor tab on the editor step (last button reads "Done"), that finishing gates + hides, that once gated `start(false)` is a no-op / `start(true)` replays / skip also gates, that the cheat-sheet button launches it, and that with the gate set it does NOT auto-appear on reload. |
...
| 93 | Workflow thumbnails | Show what "Bleach Bypass" does. | ✅ | `FFWfThumb` — every workflow card gets a baked static thumbnail: the workflow's look applied to a canonical reference frame (colour bars + skin patch + gradient, shared with hover-preview via `FFHoverPreview.testFrame`). `renderThumb(canvas, wf)` reuses the #94 engine (`deriveLook` → ops, `applyLook`); audio-only workflows get a waveform tile. `decorate(grid)` injects one `.wf-thumb` canvas per card and a `MutationObserver` re-bakes them after the grid re-renders on search/category change (idempotent). `.test/wf-thumbs.mjs` (5/5) verifies on real pixels: a grayscale workflow → grayscale tile, a saturate workflow widens the spread vs the canon, invert ≠ grayscale, an audio workflow renders a non-blank waveform, and `decorate` injects exactly one non-blank thumbnail per card across all 227 cards without duplicating on a second pass. |
| 94 | Hover-preview a workflow on the canvas | Preview before committing. | ✅ | `FFHoverPreview` — hovering a workflow card paints a fast *approximation* of that workflow's look on a floating overlay canvas. The look is DERIVED, not hand-keyed: `deriveLook(wf)` first parses the workflow's real generated ffmpeg command (`eq=saturation/contrast/brightness/gamma`, `hue=s=0`, `negate`, `curves=`, `vignette`, `noise=`, `colortemperature`, `gblur`/`boxblur`, `unsharp`) then falls back to keyword heuristics on name/tags (noir→grayscale, bleach, cyberpunk, vintage, warm/cool, vibrant, poster, blur…). Each match becomes pure per-pixel ops that `applyLook` runs over an ImageData; audio-only workflows report `audio:true` (no visual). Honest "≈ approximate preview" label. `.test/hover-preview.mjs` (6/6) verifies the ops on real canvas pixels (grayscale→R=G=B, invert→255−x, saturate widens spread, warm gain lifts R over B), that the look is grounded in the real command and in keywords, that 100% of video workflows resolve to ≥1 op, and end-to-end that `showFor` paints the overlay grayscale for a mono workflow and that hovering a real card reveals the overlay. |
